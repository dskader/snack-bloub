...
- **Two sources of shapes, not to be mixed.** `profiles.ts` is generated from the
  ...
  animation.
- **A shape can carry a `head` (`head.ts`), and only the three Snack heads do**: `snack`
  (the logo), `snack-sans-bouche` (the mirrored, mouthless variant) and `snack-bouclier`
  (symmetric, frontal, mouthless, drawn without eyes). A head may add a
  `mouth` (a third hole in the mask, next to the eyes) with a jaw (`Pose.jaw`, 0–1: open
  profiles measured off the image at 1/3, 2/3 and 1, the mouth's bottom edge moved by the
  same hinge) — the two mouthless heads have none, so `Pose.jaw` does nothing on them and
  their eyes grow from their centre. Every head carries a face anchor that remaps
  EVERY face on that body — expressions, wink, wide, notify, pointer tracking — around
  the logo's eyes: absolute gaze plus `gain` × the gap to neutral, damped drift
  (`wander`), damped growth past neutral size (`growth`), bigger eyes growing upwards.
  Both fade with one weight (shape morph × base-body state). Heads have no `eyefit`
  entry: the anchor already places the eyes. `snack.ts` is generated by
  `tools/extract-snack.py` from `tools/snack-logo-reference.png`, and `snack-no-mouth.ts`
  by `tools/extract-snack.py --no-mouth` from `tools/snack-no-mouth-reference.png`, and
  `snack-shield.ts` by `tools/extract-snack.py --shield` from
  `tools/snack-shield-reference.png` (its alpha, upscaled 4×: the export is 213 px); don't
  edit any of them by hand. The logo and no-mouth anchors are fits on those images'
  eyes; the shield's drawing has no eyes, so its anchor is CHOSEN (no-mouth proportions,
  spaced wider because a frontal face shows both eyes at full width), and `head.test.ts`
  locks that choice. Regenerate a file and refit its anchor, or that test fails. The
  shield also damps `wander` to 0.4: at 0.6 both frontal eyes turn back together at the
  end of the drift and `skins.test.ts` counts 4 idle reversals against the circle's 2.
  `skins.test.ts` sweeps every base-body state × expression × 60 instants and fails if
  an eye touches the mouth — the logo's left eye sits a few units above a zigzag peak,
  so the anchor values are tuned to that margin (worst case 0.8 units, `effraye`).
- **`StateDef.headPose` is the state's variant on a head**: `thinking`, `sleep`, `play`,
  `alert` and `chomp` declare one. The engine swaps in the head's profile, blends the
  variant with `pose` by the head's weight (so a shape change mid-state morphs), and the
  mouth and anchor apply there too. Head poses place the eyes with `Pose.eyeShift`, not
  with gaze: the anchor's `gain` would damp a gaze swing to nothing. `orbit` and `burst`
  deliberately have none — their rings and particles are laid out around a ball. These
  variants and `chomp` are CHOSEN, not measured; `chomp` is in `SEQUENCE` (15 states) as
  the one Snack addition to the catalogue, and works on every shape (the ball and the
  mouthless heads stretch and squash instead of opening a jaw). `headPose` receives the
  head, which is how `chomp` knows whether there is a jaw to open.
