...
- **Two sources of shapes, not to be mixed.** `profiles.ts` is generated from the
  ...
  animation.
- **A shape can carry a `head` (`head.ts`), and only the Snack head does.** It adds a
  mouth (a third hole in the mask, next to the eyes), a jaw (`Pose.jaw`, 0–1: open
  profiles measured off the image at 1/3, 2/3 and 1, the mouth's bottom edge moved by the
  same hinge), and a face anchor that remaps
  ...
  `tools/extract-snack.py` from `tools/snack-logo-reference.png`; don't edit it by hand.
...
- **`StateDef.headPose` is the state's variant on a head**: `thinking`, `sleep`, `play`,
  ...
  the one Snack addition to the catalogue, and works on every shape (the ball stretches
  and squashes instead of opening a jaw).
