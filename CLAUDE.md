...
- **Two sources of shapes, not to be mixed.** `profiles.ts` is generated from the
  ...
  animation.
- **A shape can carry a `head` (`head.ts`), and only the two Snack heads do**: `snack`
  (the logo) and `snack-sans-bouche` (the mirrored, mouthless variant). A head may add a
  ...
  same hinge) — `snack-sans-bouche` has none, so `Pose.jaw` does nothing on it and its
  eyes grow from their centre. Every head carries a face anchor that remaps
  ...
  by `tools/extract-snack.py --no-mouth` from `tools/snack-no-mouth-reference.png`; don't
  edit either by hand. The anchors are fits on those images, locked by `head.test.ts`:
  regenerate a file and refit its anchor, or that test fails.
...
- **`StateDef.headPose` is the state's variant on a head**: `thinking`, `sleep`, `play`,
  ...
  mouthless head stretch and squash instead of opening a jaw). `headPose` receives the
